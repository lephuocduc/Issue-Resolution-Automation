...
function Invoke-Script {
    param ($scriptPath)
    try {
        # Determine the base directory of the current script or EXE
        $baseDir = if ($MyInvocation.MyCommand.Path) {
            # If the script is run from an EXE, get its directory
            [System.IO.Path]::GetDirectoryName($MyInvocation.MyCommand.Path)
        } else {
            # For normal .ps1 script, use the script root
            $PSScriptRoot
        }

        # Build the script path dynamically
        $fullScriptPath = Join-Path $baseDir 'Scripts\LowFreeSpace.ps1'
        ...
        $arguments = "-NoProfile -ExecutionPolicy Bypass -File `"$fullScriptPath`" -ServerName `"$serverName`""
